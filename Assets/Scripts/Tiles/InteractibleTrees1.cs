...
public class InteractibleTrees1 : MonoBehaviour, IPointerDownHandler
{

	private Image tileInfoImage;
	private TileInfo tileInfo;
	private string type;
	private Trees1Tile trees1Tile;
	...
	void Start ()
	{
		trees1Tile = GetComponent<Trees1Tile> ();
	...
	public void OnPointerDown (PointerEventData pointerEventData)
	{
		...
		try {
			...
			if (Physics.Raycast (ray, out hit)) {
				...
				if (hit.transform.GetComponent<Trees1Tile> ().typeOfTile == "trees1") {
					tileInfoImage.enabled = false;
					closeImage.enabled = false;
					closeText.enabled = false;
					deleteImage.enabled = false;
					...
					tileInfo.nameOfTile.text = trees1Tile.nameOfTile;
					tileInfo.levelOfTile.text = "Level " + (GetComponent<Trees1Tile> ().levelOfTile).ToString ();
